...
read_chunk("TC_SalesQuota_UtilityCode2.R")
read_chunk("TC_SalesQuota_Plots2.R")
```
...
The outcomes are extremely variable in the low value region implying that the quota originally set was not a good predictor. Some people are being set quotas they can't come anywhere near, while others seem to be set the low end of quotas and breeze through to a very high attainment over target.
...
The quota setting for group B seems to be overcautious, allowing certain individuals to significantly outperform expectations after being given a low quota. Group A on the other hand are not so fortunate when faced with a low quota, this often isn't set low enough to give them a fair chance of success.
...
This strong linearity this is a non-intuitive observation. Consider if this model were true. One could simply increase the quotas and expect increased sales up to infinity.    
...
This is a very simple model from simple data. It doesn't explain the clusters in density around the outcomes.

We can see a heteroscedastic trend, not in the residual plot, but in the data itself as shown in the following graph of the outcome against target vs the Quota. 

````{r ggplot_OnTarget_depends_Quota, opts.label='fig.wide'}
```

This plot zooms in on the difference between groups A and B. The outcome is the distance above or below the quota, regardless of actual sales values. Group A, with sales in parity with quota have a line that runs  horizontal along point zero. Group B do better if given a low quota and worse otherwise.

The variance (spread of the points) is very high to the left, much lower to the right. This indicates that the predictability of the outcome at the time the quotas were set was much poorer for the lower range of quotas. There was more certainty about the outcome at the high value end.  
...
The plots show the variable derived from the heteroscedastic trend and then its reciprocal (multiplied by 10 for parity with other units). The latter will be referred to as Certainty for the remainder of the report because as Certainty increases, the outcomes become less variable (better predicted by the quota). The units have been ommitted because they are arbritrary. This variable is derived from a trace pattern in the data but the cause of that pattern is not known.
...
```{r bwplot_cluster_analysis, opts.label='fig.wide'}
```

The box plots show the effect of the Certainty variable, as the view travels from underperformance in cluster 1 to high performance in cluster 3. Random variation would be distributed evenly above and below the mean. In these plots, it appears to be the case that uncertainty led to better results overall for a small group of individuals. On the whole, they had the lowest quotas to start with. These quotas appear to have been biased downwards, enabling the sales agents who received them to achieve around 5% better results at the end of the year.

There could be any number of reasons for this. Retuning to the working theory; the Sales manager or VP's responsibility is to mediate an achievable target based on some measure of how easy or difficult a customer is. There's a reason the Sales vs Quota plots an almost perfect 1:1 regression line.

Intuitively, erring on the side of caution seems a plausible explanation. The VP knows these are really difficult customers and undercooks the quota out of a sense of fairness. Firm negotiation from the sales agents who have to work with these difficult clients is another possibility. Deliberate bias can't be ruled out, but there can be no suggestion of if from the available data.
