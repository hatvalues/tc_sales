...
Compare the results with Group A. An arbritrary split has been applied by Quota to get bands bands containing equal numbers of individuals. Only the lower three bands are shown as they roughly approximate the Group B clusters and above this quota level, there is only high Certainty.

```{r bwplot_compare_analysis, opts.label='fig.relaxed'}
```

Here, there is a different trend. Where there was high  Certainty at quota setting, these were shown to be achievable, shown by the relevant boxes centred around zero.

The contrast is where those who diverged from their targets trend well below (median at -5 percentage points).
