...
The data available are very limited and there are many questions that cannot be answered. This report seeks to determine what can be inferred based only on the available data.
...
This report concludes that quota rigging is extremely unlikely to have taken place. However, the quota setting process is increasingly inaccurate with lower values of quota and the outcome of this is some very good and some very poor attainments on target. The split is even between these extremes and it's not possible to reach any other conclusion without much more information about individual cases.
...
Alternatively, \(e_i\) is the deviation from the predicted sales \(\hat{Y}_i\), given Quota \(X_i\). 

The task at hand is to try to determine if there are any discernable patterns to this deviation \(e\), and if the combination of Quota and Group could apply any influence.
...
For Group B the picture is not so clear. In the shadow of much higher variance, there appears to be a different trend; Those with lower quotas may be finding it easier to meet and exceed the attainment while those with higher quotas struggle. This pattern is less clear and the regression line may be heavily influenced by a small number of significant outliers.

There is a pattern of heteroscedasticity over the whole population, whereby the lower quotas are much poorer estimators of sales than the higher quotas. 
...
## Linear Modeling
Attainment is the unit free function of sales and is taken as the dependent variable, based on the assumption that quotas are set to drive sales. The Quota data is centred for each group with \(\bar{X} - \mu\).

The other independent variable is the Group asignment, either A or B.
...
This report concludes that while there is no clear evidence rigging of the quotas, the quota setting process is failing as a predictor of attainment for Group B. At the lower range of quotas, attainments are out by \(\pm50\) percentage points. 
...
Without any information about how the customers are modeled, the linear relationship is assumed to be this intuited process combined with the log relationship all rolled up into the value of \(\beta_1\) of the linear model. No judgements can be made with the available data about how fair the the process of quota setting is at this level of detail.
