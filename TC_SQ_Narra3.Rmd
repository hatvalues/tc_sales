...
We create a series Bayesian logistic models to regress the boolean outcome "Met Target" on the Group and KQuota variables, including testing for interactions (different slopes). The priors are set according to the coefficients of a generalized linear model (not shown).

```{r model3els, opts.label='fig.tile'}
model1 <- MCMClogit(I(MetTarget * 1) ~ 1,
                      data=raw.data, b0=c(-0.2),
                      B0=c(0.3),
                      marginal.likelihood="Laplace", mcmc=10000)

model2 <- MCMClogit(I(MetTarget * 1) ~ KQuota + Group,
                      data=raw.data, b0=c(-0.2, 0, 1.1),
                      B0=c(0.3, 0.1, 0.8),
                      marginal.likelihood="Laplace", mcmc=10000)

model3 <- MCMClogit(I(MetTarget * 1) ~ KQuota * Group,
                      data=raw.data, b0=c(-0.2, 0, 1.1, -0.1),
                      B0=c(0.3, 0.1, 0.8, 0.05),
                      marginal.likelihood="Laplace", mcmc=10000)
...
From these results, we can see that zero is centrally located in the credible interval for the group A slope (quota "main" effect), so this can be assumed to be negligible. For the posterior distributions of the group B intercept and slope parameters, zero is outside or almost outside of the 95% HDI. We interpret this informally as an indication that of how likely it is that group B members with a low quota have a greater probability of hitting target - the negative slope indicating that increasing quotas decreases this probability. We might say that the lowest quotas are not as challenging, but only for group B members. The following plot illustrates this result by performing the inverse logit on each of the parameter sets in the MCMC. The median prediction, inter-quartile and and 95% credible intervals are shown for each instance. Redundant marks "T" and "F" show the true outcomes of MetTarget True/False and the top/bottom of the panel for the group B members.
...
In light of this finding we suggest that it is plausible that individuals in group B with the lowest quotas may be working under a less challenging regime than others in their group and in the department as a whole. The evidence for this is not absolutely conclusive, but a review of the quota setting for group B may be advisable.
