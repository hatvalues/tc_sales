...
```{r Exploratory_Graph_Sales_depends_Quota, opts.label='fig.wide'}
...
```{r Exploratory_Graph_simSales_depends_Quota, opts.label='fig.wide'}
set.seed(3334)
coefs <- round(lm.mvar$coefficients,4)
raw.data$simTarget <- rnorm(n, coefs[1] + 
                               coefs[2] * rollingSD +
                             #(Group == "A") * 1 * coefs[3] * Quota +
...
qplot(predict(lm.mvar, raw.data) + (Group == "B") * 4.5, OnTarget, colour = Group) +
  theme_bw()
qplot(predict(lm1, raw.data), OnTarget, colour = Group) +
  theme_bw()
